...
def setup_platform(hass, config, add_entities, discovery_info=None):
    """Set up the PCF8574 binary sensors."""
    ...
    for pin_num, pin_name in pins.items():
        pin = pcf.get_pin(pin_num // 8, pin_num % 8)
        binary_sensors.append(
            PCF8574BinarySensor(pin_name, pin, pull_mode, invert_logic)
        )

    add_entities(binary_sensors, True)


class PCF8574BinarySensor(BinarySensorDevice):
    """Represent a binary sensor that uses PCF8574."""

    def __init__(self, name, pin, pull_mode, invert_logic):
        """Initialize the PCF8574 binary sensor."""
